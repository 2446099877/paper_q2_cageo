...
To support editorial and reviewer inspection, we have prepared a reviewer-safe code packet containing the split manifests, experiment scripts, configurations, source modules, generated manuscript tables, and reproduction notes. The final public repository will be released under the `MIT License`, and the manuscript's `Code availability` section is written to align with that release plan.
